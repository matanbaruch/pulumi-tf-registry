...
export class KubernetesAssurancePolicy extends pulumi.CustomResource {
    ...
    public readonly autoScanTimes!: pulumi.Output<outputs.KubernetesAssurancePolicyAutoScanTime[] | undefined>;
    /**
     ...
     */
    public readonly kubernetesControls!: pulumi.Output<outputs.KubernetesAssurancePolicyKubernetesControl[] | undefined>;
    public readonly kubernetesControlsAvdIds!: pulumi.Output<string[] | undefined>;
    /**
     * List of kubernetes control names and available kubernetes controls are: 'Access to host IPC namespace', 'Access to host
     * PID', 'Access to host network', 'Access to host ports', 'All container images must start with a GCR domain', 'All
     * container images must start with an ECR domain', 'All container images must start with the *.azurecr.io domain', 'CPU
     * not limited', 'CPU requests not specified', 'Can elevate its own privileges', 'ConfigMap with secrets', 'ConfigMap with
     * sensitive content', 'Container images from public registries used', 'Default capabilitiessome containers do not drop
     * all', 'Default capabilitiessome containers do not drop any', 'Delete pod logs', 'Exec into Pods', 'Image tag :latest
     * used', 'Manage EKS IAM Auth ConfigMap', 'Manage Kubernetes RBAC resources', 'Manage Kubernetes networking', 'Manage
     * Kubernetes workloads and pods', 'Manage all resources', 'Manage all resources at the namespace', 'Manage configmaps',
     * 'Manage namespace secrets', 'Manage secrets', 'Manage webhookconfigurations', 'Manages /etc/hosts', 'Memory not
     * limited', 'Memory requests not specified', 'Non-core volume types used.', 'Non-default /proc masks set', 'Privileged',
     * 'Root file system is not read-only', 'Runs as root user', 'Runs with GID <= 10000', 'Runs with UID <= 10000', 'Runs with
     * a root primary or supplementary GID', 'Runtime/Default AppArmor profile not set', 'Runtime/Default Seccomp profile not
     * set', 'SELinux custom options set', 'SYS_ADMIN capability added', 'Seccomp policies disabled', 'Service with External
     * IP', 'Specific capabilities added', 'Unsafe sysctl options set', 'User with admin access', 'Workloads in the default
     * namespace', 'hostPath volume mounted with docker.sock', 'hostPath volumes mounted'
     */
    ...
    public readonly policySettings!: pulumi.Output<outputs.KubernetesAssurancePolicyPolicySettings | undefined>;
    ...
    public readonly scopes!: pulumi.Output<outputs.KubernetesAssurancePolicyScope[] | undefined>;
    /**
    ...
    constructor(name: string, argsOrState?: KubernetesAssurancePolicyArgs | KubernetesAssurancePolicyState, opts?: pulumi.CustomResourceOptions) {
        ...
        super(KubernetesAssurancePolicy.__pulumiType, name, resourceInputs, opts, false /*dependency*/, utilities.getPackage());
    }
}
