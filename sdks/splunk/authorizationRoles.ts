...
export class AuthorizationRoles extends pulumi.CustomResource {
    ...
    /**
     * Specify a role to import attributes from. To import multiple roles, specify them separately. By default a role imports
     * no other roles. Importing other roles imports all aspects of that role, such as capabilities and allowed indexes to
     * search. In combining multiple roles, the effective value for each attribute is the value with the broadest
     * permissions.nDefault roles: admin, can_delete, power, user. You can specify additional roles created.
     */
    ...
    /**
     * Specify the maximum number of concurrent real-time search jobs for this role. This count is independent from the normal
     * search jobs limit.
     */
    public readonly realtimeSearchJobsQuota!: pulumi.Output<number | undefined>;
    /**
     * Specifies the maximum disk space in MB that can be used by a user's search jobs. For example, a value of 100 limits this
     * role to 100 MB total.
     */
    public readonly searchDiskQuota!: pulumi.Output<number | undefined>;
    /**
     * Specify a search string that restricts the scope of searches run by this role. Search results for this role only show
     * events that also match the search string you specify. In the case that a user has multiple roles with different search
     * filters, they are combined with an OR.
     */
    public readonly searchFilter!: pulumi.Output<string>;
    /**
     * List of indexes that this role has permissions to search. These may be wildcarded, but the index name must begin with an
     * underscore to match internal indexes.Search indexes available by default include the following. All internal indexes All
     * non-internal indexes _audit _blocksignature _internal _thefishbucket history main You can also specify other search
     * indexes added to the server.
     */
    public readonly searchIndexesAlloweds!: pulumi.Output<string[]>;
    /**
     * For this role, list of indexes to search when no index is specified. These indexes can be wildcarded, with the exception
     * that '*' does not match internal indexes. To match internal indexes, start with '_'. All internal indexes are
     * represented by '_*'. A user with this role can search other indexes using "index= " For example, "index=special_index".
     * Search indexes available by default include the following. All internal indexes All non-internal indexes _audit
     * _blocksignature _internal _thefishbucket history main other search indexes added to the server
     */
    public readonly searchIndexesDefaults!: pulumi.Output<string[]>;
    /**
     * The maximum number of concurrent searches a user with this role is allowed to run. For users with multiple roles, the
     * maximum quota value among all of the roles applies.
     */
    public readonly searchJobsQuota!: pulumi.Output<number | undefined>;
    /**
     * Maximum time span of a search, in seconds. By default, searches are not limited to any specific time window. To override
     * any search time windows from imported roles, set srchTimeWin to '0', as the 'admin' role does.
     */
    ...
    constructor(name: string, argsOrState?: AuthorizationRolesArgs | AuthorizationRolesState, opts?: pulumi.CustomResourceOptions) {
        ...
        super(AuthorizationRoles.__pulumiType, name, resourceInputs, opts, false /*dependency*/, utilities.getPackage());
    }
}
