...
export class SavedSearches extends pulumi.CustomResource {
    ...
    public readonly acl!: pulumi.Output<outputs.SavedSearchesAcl | undefined>;
    /**
    ...
    /**
     * The state of the email action. Read-only attribute. Value ignored on POST. Use actions to specify a list of enabled
     * actions. Defaults to 0.
     */
    public /*out*/ readonly actionEmail!: pulumi.Output<boolean>;
    /**
     * The password to use when authenticating with the SMTP server. Normally this value is set when editing the email
     * settings, however you can set a clear text password here and it is encrypted on the next platform restart.Defaults to
     * empty string.
     */
    public readonly actionEmailAuthPassword!: pulumi.Output<string>;
    /**
     * The username to use when authenticating with the SMTP server. If this is empty string, no authentication is attempted.
     * Defaults to empty stringNOTE: Your SMTP server might reject unauthenticated emails.
     */
    ...
    /**
     * The search command (or pipeline) which is responsible for executing the action.Generally the command is a template
     * search pipeline which is realized with values from the saved search. To reference saved search field values wrap them in
     * $, for example to reference the savedsearch name use $name$, to reference the search use $search$.
     */
    public readonly actionEmailCommand!: pulumi.Output<string>;
    /**
     * Valid values: (table | plain | html | raw | csv)Specify the format of text in the email. This value also applies to any
     * attachments.
     */
    public readonly actionEmailFormat!: pulumi.Output<string>;
    /**
     * Email address from which the email action originates.Defaults to splunk@$LOCALHOST or whatever value is set in
     * alert_actions.conf.
     */
    public readonly actionEmailFrom!: pulumi.Output<string>;
    /**
     * Sets the hostname used in the web link (url) sent in email actions.This value accepts two forms:hostname (for example,
     * splunkserver, splunkserver.example.com)
     */
    ...
    /**
     * Indicates whether the search results are contained in the body of the email.Results can be either inline or attached to
     * an email.
     */
    public readonly actionEmailInline!: pulumi.Output<boolean>;
    /**
     * Set the address of the MTA server to be used to send the emails.Defaults to <LOCALHOST> or whatever is set in
     * alert_actions.conf.
     */
    ...
    /**
     * Valid values are Integer[m|s|h|d].Specifies the maximum amount of time the execution of an email action takes before the
     * action is aborted. Defaults to 5m.
     */
    ...
    /**
     * Search string to preprocess results before emailing them. Defaults to empty string (no preprocessing).Usually the
     * preprocessing consists of filtering out unwanted internal fields.
     */
    public readonly actionEmailPreprocessResults!: pulumi.Output<string>;
    /**
     * Space-separated list. Specifies the set (and load order) of CID fonts for handling Simplified Chinese(gb), Traditional
     * Chinese(cns), Japanese(jp), and Korean(kor) in Integrated PDF Rendering.If multiple fonts provide a glyph for a given
     * character code, the glyph from the first font specified in the list is used.To skip loading any CID fonts, specify the
    ...
    /**
     * Indicates whether to attach the search results in the email.Results can be either attached or inline. See
     * action.email.inline.
     */
    ...
    /**
     * A comma or semicolon separated list of recipient email addresses. Required if this search is scheduled and the email
    ...
    /**
     * Valid values are Integer[p].Specifies the minimum time-to-live in seconds of the search artifacts if this action is
     * triggered. If p follows <Integer>, int is the number of scheduled periods. Defaults to 86400 (24 hours).If no actions
     * are triggered, the artifacts have their ttl determined by dispatch.ttl in savedsearches.conf.
     */
    ...
    /**
     * Indicates whether to use TLS (transport layer security) when communicating with the SMTP server (starttls).Defaults to
     * false.
     */
    ...
    /**
     * The state of the populate lookup action. Read-only attribute. Value ignored on POST. Use actions to specify a list of
     * enabled actions. Defaults to 0.
     */
    ...
    /**
     * Sets the hostname used in the web link (url) sent in alert actions.This value accepts two forms: hostname (for example,
     * splunkserver, splunkserver.example.com) protocol://hostname:port (for example, http://splunkserver:8000,
    ...
    /**
     * Valid values are: Integer[m|s|h|d]Sets the maximum amount of time the execution of an action takes before the action is
     * aborted. Defaults to 5m.
     */
    ...
    /**
     * Valid values are Integer[p]Specifies the minimum time-to-live in seconds of the search artifacts if this action is
     * triggered. If p follows Integer, then this specifies the number of scheduled periods. Defaults to 10p.
     */
    public readonly actionPopulateLookupTtl!: pulumi.Output<string>;
    /**
     * The state of the rss action. Read-only attribute. Value ignored on POST.Use actions to specify a list of enabled
     * actions. Defaults to 0.
     */
    ...
    /**
     * Valid values are: Integer[p] Specifies the minimum time-to-live in seconds of the search artifacts if this action is
     * triggered. If p follows Integer, specifies the number of scheduled periods. Defaults to 86400 (24 hours).
     */
    public readonly actionRssTtl!: pulumi.Output<string>;
    /**
     * The state of the script action. Read-only attribute. Value ignored on POST. Use actions to specify a list of enabled
     * actions. Defaults to 0.
     */
    ...
    /**
     * Show one or more fields from the search results below your Slack message. Comma-separated list of field names. Allows
     * wildcards. eg. index,source*
     */
    public readonly actionSlackParamFields!: pulumi.Output<string | undefined>;
    /**
     * Enter the chat message to send to the Slack channel. The message can include tokens that insert text based on the
    ...
    /**
     * The state of the summary index action. Read-only attribute. Value ignored on POST. Use actions to specify a list of
     * enabled actions. Defaults to 0.
     */
    ...
    /**
     * Sets the hostname used in the web link (url) sent in summary-index alert actions.This value accepts two forms:hostname
     * (for example, splunkserver, splunkserver.example.com)protocol://hostname:port (for example, http://splunkserver:8000,
     * https://splunkserver.example.com:443)
     */
    public readonly actionSummaryIndexHostname!: pulumi.Output<string>;
    /**
     * Determines whether to execute the summary indexing action as part of the scheduled search.NOTE: This option is
     * considered only if the summary index action is enabled and is always executed (in other words, if counttype =
    ...
    /**
     * One of the following strings: greater than, less than, equal to, rises by, drops by, rises by perc, drops by percUsed
     * with alert_threshold to trigger alert actions.
     */
    ...
    /**
     * Valid values: [number][time-unit]Sets the period of time to show the alert in the dashboard. Defaults to 24h.Use
     * [number][time-unit] to specify a time. For example: 60 = 60 seconds, 1m = 1 minute, 1h = 60 minutes = 1 hour.
     */
    public readonly alertExpires!: pulumi.Output<string>;
    /**
     * Valid values: (1 | 2 | 3 | 4 | 5 | 6) Sets the alert severity level.Valid values are:1 DEBUG 2 INFO 3 WARN 4 ERROR 5
     * SEVERE 6 FATAL Defaults to 3.
     */
    ...
    /**
     * Comma delimited list of fields to use for suppression when doing per result alerting. Required if suppression is turned
     * on and per result alerting is enabled.
     */
    public readonly alertSuppressFields!: pulumi.Output<string>;
    /**
     * Valid values: [number][time-unit] Specifies the suppresion period. Only valid if alert.supress is enabled.Use
     * [number][time-unit] to specify a time. For example: 60 = 60 seconds, 1m = 1 minute, 1h = 60 minutes = 1 hour.
     */
    public readonly alertSuppressPeriod!: pulumi.Output<string>;
    /**
     * Valid values are: Integer[%]Specifies the value to compare (see alert_comparator) before triggering the alert actions.
    ...
    /**
     * Valid values: (true | false | auto) Specifies whether to track the actions triggered by this scheduled search.auto -
     * determine whether to track or not based on the tracking setting of each action, do not track scheduled searches that
     * always trigger actions. Default value true - force alert tracking.false - disable alert tracking for this search.
     */
    public readonly alertTrack!: pulumi.Output<boolean>;
    /**
     * What to base the alert on, overriden by alert_condition if it is specified. Valid values are: always, custom, number of
     * events, number of hosts, number of sources.
     */
    public readonly alertType!: pulumi.Output<string>;
    /**
     * Allows the search scheduler to distribute scheduled searches randomly and more evenly over their specified search
     * periods.
     */
    ...
    /**
     * Cron schedule that probes and generates the summaries for this saved search.The default value is *&#47;10 * * * * and
     * corresponds to "every ten hours".
     */
    public readonly autoSummarizeCronSchedule!: pulumi.Output<string>;
    /**
     * A time string that specifies the earliest time for summarizing this search. Can be a relative or absolute time.If this
     * value is an absolute time, use the dispatch.time_format to format the value.
     */
    public readonly autoSummarizeDispatchEarliestTime!: pulumi.Output<string>;
    /**
     * A time string that specifies the latest time for summarizing this saved search. Can be a relative or absolute time.If
     * this value is an absolute time, use the dispatch.time_format to format the value.
     */
    ...
    /**
     * The maximum number of buckets with the suspended summarization before the summarization search is completely stopped,
    ...
    /**
     * The maximum ratio of summary_size/bucket_size, which specifies when to stop summarization and deem it unhelpful for a
     * bucket. Defaults to 0.1 Note: The test is only performed if the summary size is larger than
     * auto_summarize.max_summary_size.
     */
    public readonly autoSummarizeMaxSummaryRatio!: pulumi.Output<number>;
    /**
     * The minimum summary size, in bytes, before testing whether the summarization is helpful.The default value is 52428800
     * and is equivalent to 5MB.
     */
    public readonly autoSummarizeMaxSummarySize!: pulumi.Output<number>;
    /**
     * Maximum time (in seconds) that the summary search is allowed to run. Defaults to 3600.Note: This is an approximate time.
     * The summary search stops at clean bucket boundaries.
     */
    public readonly autoSummarizeMaxTime!: pulumi.Output<number>;
    /**
     * Time specfier indicating when to suspend summarization of this search if the summarization is deemed unhelpful.Defaults
     * to 24h.
     */
    public readonly autoSummarizeSuspendPeriod!: pulumi.Output<string>;
    /**
     * The list of time ranges that each summarized chunk should span. This comprises the list of available granularity levels
     * for which summaries would be available. Specify a comma delimited list of time specifiers.For example a timechart over
     * the last month whose granuality is at the day level should set this to 1d. If you need the same data summarized at the
     * hour level for weekly charts, use: 1h,1d.
     */
    public readonly autoSummarizeTimespan!: pulumi.Output<string>;
    /**
     * Valid values: cron stringThe cron schedule to execute this search. For example: *&#47;5 * * * * causes the search to execute
     * every 5 minutes.
     */
    ...
    /**
     * A time string that specifies the earliest time for this search. Can be a relative or absolute time.If this value is an
     * absolute time, use the dispatch.time_format to format the value.
     */
    public readonly dispatchEarliestTime!: pulumi.Output<string>;
    /**
     * A time string that specifies the latest time for this search. Can be a relative or absolute time.If this value is an
     * absolute time, use the dispatch.time_format to format the value.
     */
    ...
    /**
     * A time string that specifies the latest time for this saved search. Can be a relative or absolute time.If this value is
     * an absolute time, use the dispatch.time_format to format the value.
     */
    ...
    /**
     * Whether to back fill the real time window for this search. Parameter valid only if this is a real time search. Defaults
     * to 0.
     */
    ...
    /**
     * Specifies whether a new search process spawns when this saved search is executed. Defaults to 1. Searches against
    ...
    /**
     * Valid values: Integer[p]. Defaults to 2p.Indicates the time to live (in seconds) for the artifacts of the scheduled
     * search, if no actions are triggered.
     */
    public readonly dispatchTtl!: pulumi.Output<string>;
    /**
     * Defines the default UI view name (not label) in which to load the results. Accessibility is subject to the user having
    ...
    /**
     * Defaults to 1. Controls the way the scheduler computes the next execution time of a scheduled search. If this value is
     * set to 1, the scheduler bases its determination of the next scheduled search execution time on the current time. If this
     * value is set to 0, the scheduler bases its determination of the next scheduled search on the last search execution time.
     * This is called continuous scheduling. If set to 0, the scheduler never skips scheduled execution periods. However, the
     * execution of the saved search might fall behind depending on the scheduler load. Use continuous scheduling whenever you
     * enable the summary index option.
     */
    ...
    /**
     * Specifies whether to restart a real-time search managed by the scheduler when a search peer becomes available for this
     * saved search. Defaults to 1.
     */
    public readonly restartOnSearchpeerAdd!: pulumi.Output<boolean>;
    /**
     * Indicates whether this search runs at startup. If it does not run on startup, it runs at the next scheduled time.
    ...
    /**
     * Time window (in minutes) during which the search has lower priority. Defaults to 0. The scheduler can give higher
     * priority to more critical searches during this window. The window must be smaller than the search period.Set to auto to
     * let the scheduler determine the optimal window value automatically. Requires the edit_search_schedule_window capability
     * to override auto.
     */
    ...
    constructor(name: string, argsOrState?: SavedSearchesArgs | SavedSearchesState, opts?: pulumi.CustomResourceOptions) {
        ...
        super(SavedSearches.__pulumiType, name, resourceInputs, opts, false /*dependency*/, utilities.getPackage());
    }
}
