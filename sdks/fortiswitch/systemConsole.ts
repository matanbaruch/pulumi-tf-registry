// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as utilities from "./utilities";

export class SystemConsole extends pulumi.CustomResource {
    /**
     * Get an existing SystemConsole resource's state with the given name, ID, and optional extra
     * properties used to qualify the lookup.
     *
     * @param name The _unique_ name of the resulting resource.
     * @param id The _unique_ provider ID of the resource to lookup.
     * @param state Any extra arguments used during the lookup.
     * @param opts Optional settings to control the behavior of the CustomResource.
     */
    public static get(name: string, id: pulumi.Input<pulumi.ID>, state?: SystemConsoleState, opts?: pulumi.CustomResourceOptions): SystemConsole {
        return new SystemConsole(name, <any>state, { ...opts, id: id });
    }

    /** @internal */
    public static readonly __pulumiType = 'fortiswitch:index/systemConsole:SystemConsole';

    /**
     * Returns true if the given object is an instance of SystemConsole.  This is designed to work even
     * when multiple copies of the Pulumi SDK have been loaded into the same process.
     */
    public static isInstance(obj: any): obj is SystemConsole {
        if (obj === undefined || obj === null) {
            return false;
        }
        return obj['__pulumiType'] === SystemConsole.__pulumiType;
    }

    public readonly baudrate!: pulumi.Output<string>;
    public readonly hostnameDisplayLength!: pulumi.Output<number>;
    public readonly login!: pulumi.Output<string>;
    public readonly mode!: pulumi.Output<string>;
    public readonly output!: pulumi.Output<string>;

    /**
     * Create a SystemConsole resource with the given unique name, arguments, and options.
     *
     * @param name The _unique_ name of the resource.
     * @param args The arguments to use to populate this resource's properties.
     * @param opts A bag of options that control this resource's behavior.
     */
    constructor(name: string, args?: SystemConsoleArgs, opts?: pulumi.CustomResourceOptions)
    constructor(name: string, argsOrState?: SystemConsoleArgs | SystemConsoleState, opts?: pulumi.CustomResourceOptions) {
        let resourceInputs: pulumi.Inputs = {};
        opts = opts || {};
        if (opts.id) {
            const state = argsOrState as SystemConsoleState | undefined;
            resourceInputs["baudrate"] = state ? state.baudrate : undefined;
            resourceInputs["hostnameDisplayLength"] = state ? state.hostnameDisplayLength : undefined;
            resourceInputs["login"] = state ? state.login : undefined;
            resourceInputs["mode"] = state ? state.mode : undefined;
            resourceInputs["output"] = state ? state.output : undefined;
        } else {
            const args = argsOrState as SystemConsoleArgs | undefined;
            resourceInputs["baudrate"] = args ? args.baudrate : undefined;
            resourceInputs["hostnameDisplayLength"] = args ? args.hostnameDisplayLength : undefined;
            resourceInputs["login"] = args ? args.login : undefined;
            resourceInputs["mode"] = args ? args.mode : undefined;
            resourceInputs["output"] = args ? args.output : undefined;
        }
        opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts);
        super(SystemConsole.__pulumiType, name, resourceInputs, opts, false /*dependency*/, utilities.getPackage());
    }
}

/**
 * Input properties used for looking up and filtering SystemConsole resources.
 */
export interface SystemConsoleState {
    baudrate?: pulumi.Input<string>;
    hostnameDisplayLength?: pulumi.Input<number>;
    login?: pulumi.Input<string>;
    mode?: pulumi.Input<string>;
    output?: pulumi.Input<string>;
}

/**
 * The set of arguments for constructing a SystemConsole resource.
 */
export interface SystemConsoleArgs {
    baudrate?: pulumi.Input<string>;
    hostnameDisplayLength?: pulumi.Input<number>;
    login?: pulumi.Input<string>;
    mode?: pulumi.Input<string>;
    output?: pulumi.Input<string>;
}
