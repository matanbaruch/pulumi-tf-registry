...
export class ProcessGroupDetectionFlags extends pulumi.CustomResource {
    ...
    /**
     * In older versions, Node.js applications were distinguished based on their directory name, omitting the script name.
    ...
    /**
     * Enabling this flag will detect webMethods Integration Server including specific properties like install root and product
     * name.
     */
    ...
    /**
     * Enable to group and separately analyze the processes of each IBM MQ Queue manager instance. Each process group receives
     * a unique name based on the queue manager instance name.
     */
    public readonly groupIbmmqbyInstanceName!: pulumi.Output<boolean>;
    /**
     * Enabling this flag will detect the JBoss server name from the system property jboss.server.name=<server-name>, only if
     * -D[Server:<server-name>] is not set.
     */
    public readonly identifyJbossServerBySystemProperty!: pulumi.Output<boolean>;
    /**
     * To determine the unique identity of each detected process, and to generate a unique name for each detected process,
    ...
    /**
     * Enable to monitor CPU and memory usage of short lived processes, otherwise being lost by traditional monitoring.
    ...
    /**
     * Enable to group and separately analyze the processes of each Oracle DB. Each process group receives a unique name based
     * on the Oracle DB SID.
     */
    public readonly splitOracleDatabasePg!: pulumi.Output<boolean>;
    /**
     * Enable to group and separately analyze the processes of each Oracle Listener. Each process group receives a unique name
     * based on the Oracle Listener name.
     */
    public readonly splitOracleListenerPg!: pulumi.Output<boolean>;
    /**
     * By default, Tomcat clusters are identified and named based on the CATALINA_HOME directory name. This setting results in
    ...
    /**
     * By default, Dynatrace uses image names as identifiers for individual process groups, with one process-group instance per
    ...
    constructor(name: string, argsOrState?: ProcessGroupDetectionFlagsArgs | ProcessGroupDetectionFlagsState, opts?: pulumi.CustomResourceOptions) {
        ...
        super(ProcessGroupDetectionFlags.__pulumiType, name, resourceInputs, opts, false /*dependency*/, utilities.getPackage());
    }
}
