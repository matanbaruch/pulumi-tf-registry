...
export class CustomAnomalies extends pulumi.CustomResource {
    ...
    /**
     * The reason of automatic disabling. The `NONE` means config was not disabled automatically. Possible values are
    ...
    /**
     * Defines the scope of the metric event. Only one filter is allowed per filter type, except for tags, where up to 3 are
    ...
    /**
     * The type of the event to trigger on the threshold violation. The `CUSTOM_ALERT` type is not correlated with other
     * alerts. The `INFO` type does not open a problem
     */
    public readonly severity!: pulumi.Output<string | undefined>;
    /**
     * A monitoring strategy for a metric event config. This is the base version of the monitoring strategy, depending on the
    ...
    /**
     * The reason of a warning set on the config. The `NONE` means config has no warnings. The other supported value is
     * `TOO_MANY_DIMS`
     *
     * @deprecated Deprecated
     */
    ...
    constructor(name: string, argsOrState?: CustomAnomaliesArgs | CustomAnomaliesState, opts?: pulumi.CustomResourceOptions) {
        ...
        super(CustomAnomalies.__pulumiType, name, resourceInputs, opts, false /*dependency*/, utilities.getPackage());
    }
}
