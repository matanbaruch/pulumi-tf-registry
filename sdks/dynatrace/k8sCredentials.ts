...
export class K8sCredentials extends pulumi.CustomResource {
    ...
    /**
     * Monitoring is enabled (`true`) or disabled (`false`) for given credentials configuration. If not set on creation, the
    ...
    /**
     * The service account bearer token for the Kubernetes API server. Submit your token on creation or update of the
    ...
    /**
     * The check of SSL certificates is enabled (`true`) or disabled (`false`) for the Kubernetes cluster. If not set on
    ...
    /**
     * Inclusion of all Davis relevant events is enabled (`true`) or disabled (`false`) for the Kubernetes cluster. If the
    ...
    /**
     * The URL of the Kubernetes API server. It must be unique within a Dynatrace environment. The URL must valid according to
     * RFC 2396. Leading or trailing whitespaces are not allowed.
     */
    public readonly endpointUrl!: pulumi.Output<string | undefined>;
    /**
     * Event analysis and alerting is (`true`) or disabled (`false`) for the Kubernetes cluster. If the field is omitted during
    ...
    /**
     * Kubernetes event filters based on field-selectors. If set to `null` on creation, no events field selectors are
    ...
    /**
     * Monitoring of events is enabled (`true`) or disabled (`false`) for the Kubernetes cluster. Event monitoring depends on
    ...
    /**
     * The name of the Kubernetes credentials configuration. Allowed characters are letters, numbers, whitespaces, and the
     * following characters: `.+-_`. Leading or trailing whitespace is not allowed.
     */
    public readonly label!: pulumi.Output<string>;
    /**
     * Prometheus exporters integration is enabled (`true`) or disabled (`false`) for the Kubernetes cluster.If the field is
     * omitted during an update, the old value remains unaffected
     */
    ...
    /**
     * Workload and cloud application processing is enabled (`true`) or disabled (`false`) for the Kubernetes cluster. If the
     * field is omitted during an update, the old value remains unaffected.
     */
    ...
    constructor(name: string, argsOrState?: K8sCredentialsArgs | K8sCredentialsState, opts?: pulumi.CustomResourceOptions) {
        ...
        super(K8sCredentials.__pulumiType, name, resourceInputs, opts, false /*dependency*/, utilities.getPackage());
    }
}
