...
export class ApplicationDataPrivacy extends pulumi.CustomResource {
    ...
    /**
     * (Field has overlap with `dynatrace.DataPrivacy`) Set to `true` to disable data capture and cookies until JavaScriptAPI
     * `dtrum.enable()` is called
     */
    public readonly dataCaptureOptIn!: pulumi.Output<boolean | undefined>;
    /**
     * (Field has overlap with `dynatrace.DataPrivacy`) How to handle the "Do Not Track" header: * `IGNORE_DO_NOT_TRACK`:
    ...
    /**
     * (Field has overlap with `dynatrace.DataPrivacy`) Set to `true` to set persistent cookie in order to recognize returning
     * devices
     */
    ...
    constructor(name: string, argsOrState?: ApplicationDataPrivacyArgs | ApplicationDataPrivacyState, opts?: pulumi.CustomResourceOptions) {
        ...
        super(ApplicationDataPrivacy.__pulumiType, name, resourceInputs, opts, false /*dependency*/, utilities.getPackage());
    }
}
