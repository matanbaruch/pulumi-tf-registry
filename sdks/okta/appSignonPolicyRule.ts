...
export class AppSignonPolicyRule extends pulumi.CustomResource {
    ...
    /**
     * This is an optional advanced setting. If the expression is formatted incorrectly or conflicts with conditions set above,
     * the rule may not match any users.
     */
    ...
    /**
     * If the device is managed. A device is managed if it's managed by a device management system. When managed is passed,
     * registered must also be included and must be set to true.
     */
    public readonly deviceIsManaged!: pulumi.Output<boolean | undefined>;
    /**
     * If the device is registered. A device is registered if the User enrolls with Okta Verify that is installed on the
     * device.
     */
    ...
    /**
     * The inactivity duration after which the end user must re-authenticate. Use the ISO 8601 Period format for recurring time
     * intervals.
     */
    ...
    /**
     * The duration after which the end user must re-authenticate, regardless of user activity. Use the ISO 8601 Period format
     * for recurring time intervals. PT0S - Every sign-in attempt, PT43800H - Once per session
     */
    ...
    constructor(name: string, argsOrState?: AppSignonPolicyRuleArgs | AppSignonPolicyRuleState, opts?: pulumi.CustomResourceOptions) {
        ...
        super(AppSignonPolicyRule.__pulumiType, name, resourceInputs, opts, false /*dependency*/, utilities.getPackage());
    }
}
