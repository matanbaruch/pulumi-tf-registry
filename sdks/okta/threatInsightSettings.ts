...
export class ThreatInsightSettings extends pulumi.CustomResource {
    ...
    /**
     * Specifies how Okta responds to authentication requests from suspicious IPs. Valid values are `none`, `audit`, or
     * `block`. A value of `none` indicates that ThreatInsight is disabled. A value of `audit` indicates that Okta logs
     * suspicious requests in the System Log. A value of `block` indicates that Okta logs suspicious requests in the System Log
     * and blocks the requests.
     */
    public readonly action!: pulumi.Output<string>;
    /**
     * Accepts a list of Network Zone IDs. Can only accept zones of `IP` type. IPs in the excluded Network Zones aren't logged
     * or blocked by Okta ThreatInsight and proceed to Sign On rules evaluation. This ensures that traffic from known, trusted
     * IPs isn't accidentally logged or blocked. The ordering of the network zone is not guarantee from the API sides
     */
    ...
    constructor(name: string, argsOrState?: ThreatInsightSettingsArgs | ThreatInsightSettingsState, opts?: pulumi.CustomResourceOptions) {
        ...
        super(ThreatInsightSettings.__pulumiType, name, resourceInputs, opts, false /*dependency*/, utilities.getPackage());
    }
}
