// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as utilities from "./utilities";

export class DcInstance extends pulumi.CustomResource {
    /**
     * Get an existing DcInstance resource's state with the given name, ID, and optional extra
     * properties used to qualify the lookup.
     *
     * @param name The _unique_ name of the resulting resource.
     * @param id The _unique_ provider ID of the resource to lookup.
     * @param state Any extra arguments used during the lookup.
     * @param opts Optional settings to control the behavior of the CustomResource.
     */
    public static get(name: string, id: pulumi.Input<pulumi.ID>, state?: DcInstanceState, opts?: pulumi.CustomResourceOptions): DcInstance {
        return new DcInstance(name, <any>state, { ...opts, id: id });
    }

    /** @internal */
    public static readonly __pulumiType = 'tencentcloud:index/dcInstance:DcInstance';

    /**
     * Returns true if the given object is an instance of DcInstance.  This is designed to work even
     * when multiple copies of the Pulumi SDK have been loaded into the same process.
     */
    public static isInstance(obj: any): obj is DcInstance {
        if (obj === undefined || obj === null) {
            return false;
        }
        return obj['__pulumiType'] === DcInstance.__pulumiType;
    }

    /**
     * Access point of connection.You can call `DescribeAccessPoints` to get the region ID. The selected access point must
     * exist and be available.
     */
    public readonly accessPointId!: pulumi.Output<string>;
    /**
     * Connection port bandwidth in Mbps. Value range: [2,10240]. Default value: 1000.
     */
    public readonly bandwidth!: pulumi.Output<number | undefined>;
    /**
     * Circuit code of a connection, which is provided by the ISP or connection provider.
     */
    public readonly circuitCode!: pulumi.Output<string | undefined>;
    /**
     * User-side IP address for connection debugging, which is automatically assigned by default.
     */
    public readonly customerAddress!: pulumi.Output<string | undefined>;
    /**
     * Email address of connection applicant, which is obtained from the account system by default.
     */
    public readonly customerContactMail!: pulumi.Output<string | undefined>;
    /**
     * Contact number of connection applicant, which is obtained from the account system by default.
     */
    public readonly customerContactNumber!: pulumi.Output<string | undefined>;
    /**
     * Name of connection applicant, which is obtained from the account system by default.
     */
    public readonly customerName!: pulumi.Output<string | undefined>;
    /**
     * Connection name.
     */
    public readonly directConnectName!: pulumi.Output<string>;
    /**
     * Fault reporting contact number.
     */
    public readonly faultReportContactNumber!: pulumi.Output<string | undefined>;
    /**
     * Fault reporting contact person.
     */
    public readonly faultReportContactPerson!: pulumi.Output<string | undefined>;
    /**
     * ISP that provides connections. Valid values: ChinaTelecom (China Telecom), ChinaMobile (China Mobile), ChinaUnicom
     * (China Unicom), In-houseWiring (in-house wiring), ChinaOther (other Chinese ISPs), InternationalOperator (international
     * ISPs).
     */
    public readonly lineOperator!: pulumi.Output<string>;
    /**
     * Local IDC location.
     */
    public readonly location!: pulumi.Output<string | undefined>;
    /**
     * Port type of connection. Valid values: 100Base-T (100-Megabit electrical Ethernet interface), 1000Base-T (1-Gigabit
     * electrical Ethernet interface), 1000Base-LX (1-Gigabit single-module optical Ethernet interface; 10 KM), 10GBase-T
     * (10-Gigabit electrical Ethernet interface), 10GBase-LR (10-Gigabit single-module optical Ethernet interface; 10 KM).
     * Default value: 1000Base-LX.
     */
    public readonly portType!: pulumi.Output<string>;
    /**
     * ID of redundant connection.
     */
    public readonly redundantDirectConnectId!: pulumi.Output<string | undefined>;
    /**
     * Whether the connection applicant has signed the service agreement. Default value: true.
     */
    public readonly signLaw!: pulumi.Output<boolean | undefined>;
    /**
     * Tencent-side IP address for connection debugging, which is automatically assigned by default.
     */
    public readonly tencentAddress!: pulumi.Output<string | undefined>;
    /**
     * VLAN for connection debugging, which is enabled and automatically assigned by default.
     */
    public readonly vlan!: pulumi.Output<number | undefined>;

    /**
     * Create a DcInstance resource with the given unique name, arguments, and options.
     *
     * @param name The _unique_ name of the resource.
     * @param args The arguments to use to populate this resource's properties.
     * @param opts A bag of options that control this resource's behavior.
     */
    constructor(name: string, args: DcInstanceArgs, opts?: pulumi.CustomResourceOptions)
    constructor(name: string, argsOrState?: DcInstanceArgs | DcInstanceState, opts?: pulumi.CustomResourceOptions) {
        let resourceInputs: pulumi.Inputs = {};
        opts = opts || {};
        if (opts.id) {
            const state = argsOrState as DcInstanceState | undefined;
            resourceInputs["accessPointId"] = state ? state.accessPointId : undefined;
            resourceInputs["bandwidth"] = state ? state.bandwidth : undefined;
            resourceInputs["circuitCode"] = state ? state.circuitCode : undefined;
            resourceInputs["customerAddress"] = state ? state.customerAddress : undefined;
            resourceInputs["customerContactMail"] = state ? state.customerContactMail : undefined;
            resourceInputs["customerContactNumber"] = state ? state.customerContactNumber : undefined;
            resourceInputs["customerName"] = state ? state.customerName : undefined;
            resourceInputs["directConnectName"] = state ? state.directConnectName : undefined;
            resourceInputs["faultReportContactNumber"] = state ? state.faultReportContactNumber : undefined;
            resourceInputs["faultReportContactPerson"] = state ? state.faultReportContactPerson : undefined;
            resourceInputs["lineOperator"] = state ? state.lineOperator : undefined;
            resourceInputs["location"] = state ? state.location : undefined;
            resourceInputs["portType"] = state ? state.portType : undefined;
            resourceInputs["redundantDirectConnectId"] = state ? state.redundantDirectConnectId : undefined;
            resourceInputs["signLaw"] = state ? state.signLaw : undefined;
            resourceInputs["tencentAddress"] = state ? state.tencentAddress : undefined;
            resourceInputs["vlan"] = state ? state.vlan : undefined;
        } else {
            const args = argsOrState as DcInstanceArgs | undefined;
            if ((!args || args.accessPointId === undefined) && !opts.urn) {
                throw new Error("Missing required property 'accessPointId'");
            }
            if ((!args || args.directConnectName === undefined) && !opts.urn) {
                throw new Error("Missing required property 'directConnectName'");
            }
            if ((!args || args.lineOperator === undefined) && !opts.urn) {
                throw new Error("Missing required property 'lineOperator'");
            }
            if ((!args || args.portType === undefined) && !opts.urn) {
                throw new Error("Missing required property 'portType'");
            }
            resourceInputs["accessPointId"] = args ? args.accessPointId : undefined;
            resourceInputs["bandwidth"] = args ? args.bandwidth : undefined;
            resourceInputs["circuitCode"] = args ? args.circuitCode : undefined;
            resourceInputs["customerAddress"] = args ? args.customerAddress : undefined;
            resourceInputs["customerContactMail"] = args ? args.customerContactMail : undefined;
            resourceInputs["customerContactNumber"] = args ? args.customerContactNumber : undefined;
            resourceInputs["customerName"] = args ? args.customerName : undefined;
            resourceInputs["directConnectName"] = args ? args.directConnectName : undefined;
            resourceInputs["faultReportContactNumber"] = args ? args.faultReportContactNumber : undefined;
            resourceInputs["faultReportContactPerson"] = args ? args.faultReportContactPerson : undefined;
            resourceInputs["lineOperator"] = args ? args.lineOperator : undefined;
            resourceInputs["location"] = args ? args.location : undefined;
            resourceInputs["portType"] = args ? args.portType : undefined;
            resourceInputs["redundantDirectConnectId"] = args ? args.redundantDirectConnectId : undefined;
            resourceInputs["signLaw"] = args ? args.signLaw : undefined;
            resourceInputs["tencentAddress"] = args ? args.tencentAddress : undefined;
            resourceInputs["vlan"] = args ? args.vlan : undefined;
        }
        opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts);
        super(DcInstance.__pulumiType, name, resourceInputs, opts, false /*dependency*/, utilities.getPackage());
    }
}

/**
 * Input properties used for looking up and filtering DcInstance resources.
 */
export interface DcInstanceState {
    /**
     * Access point of connection.You can call `DescribeAccessPoints` to get the region ID. The selected access point must
     * exist and be available.
     */
    accessPointId?: pulumi.Input<string>;
    /**
     * Connection port bandwidth in Mbps. Value range: [2,10240]. Default value: 1000.
     */
    bandwidth?: pulumi.Input<number>;
    /**
     * Circuit code of a connection, which is provided by the ISP or connection provider.
     */
    circuitCode?: pulumi.Input<string>;
    /**
     * User-side IP address for connection debugging, which is automatically assigned by default.
     */
    customerAddress?: pulumi.Input<string>;
    /**
     * Email address of connection applicant, which is obtained from the account system by default.
     */
    customerContactMail?: pulumi.Input<string>;
    /**
     * Contact number of connection applicant, which is obtained from the account system by default.
     */
    customerContactNumber?: pulumi.Input<string>;
    /**
     * Name of connection applicant, which is obtained from the account system by default.
     */
    customerName?: pulumi.Input<string>;
    /**
     * Connection name.
     */
    directConnectName?: pulumi.Input<string>;
    /**
     * Fault reporting contact number.
     */
    faultReportContactNumber?: pulumi.Input<string>;
    /**
     * Fault reporting contact person.
     */
    faultReportContactPerson?: pulumi.Input<string>;
    /**
     * ISP that provides connections. Valid values: ChinaTelecom (China Telecom), ChinaMobile (China Mobile), ChinaUnicom
     * (China Unicom), In-houseWiring (in-house wiring), ChinaOther (other Chinese ISPs), InternationalOperator (international
     * ISPs).
     */
    lineOperator?: pulumi.Input<string>;
    /**
     * Local IDC location.
     */
    location?: pulumi.Input<string>;
    /**
     * Port type of connection. Valid values: 100Base-T (100-Megabit electrical Ethernet interface), 1000Base-T (1-Gigabit
     * electrical Ethernet interface), 1000Base-LX (1-Gigabit single-module optical Ethernet interface; 10 KM), 10GBase-T
     * (10-Gigabit electrical Ethernet interface), 10GBase-LR (10-Gigabit single-module optical Ethernet interface; 10 KM).
     * Default value: 1000Base-LX.
     */
    portType?: pulumi.Input<string>;
    /**
     * ID of redundant connection.
     */
    redundantDirectConnectId?: pulumi.Input<string>;
    /**
     * Whether the connection applicant has signed the service agreement. Default value: true.
     */
    signLaw?: pulumi.Input<boolean>;
    /**
     * Tencent-side IP address for connection debugging, which is automatically assigned by default.
     */
    tencentAddress?: pulumi.Input<string>;
    /**
     * VLAN for connection debugging, which is enabled and automatically assigned by default.
     */
    vlan?: pulumi.Input<number>;
}

/**
 * The set of arguments for constructing a DcInstance resource.
 */
export interface DcInstanceArgs {
    /**
     * Access point of connection.You can call `DescribeAccessPoints` to get the region ID. The selected access point must
     * exist and be available.
     */
    accessPointId: pulumi.Input<string>;
    /**
     * Connection port bandwidth in Mbps. Value range: [2,10240]. Default value: 1000.
     */
    bandwidth?: pulumi.Input<number>;
    /**
     * Circuit code of a connection, which is provided by the ISP or connection provider.
     */
    circuitCode?: pulumi.Input<string>;
    /**
     * User-side IP address for connection debugging, which is automatically assigned by default.
     */
    customerAddress?: pulumi.Input<string>;
    /**
     * Email address of connection applicant, which is obtained from the account system by default.
     */
    customerContactMail?: pulumi.Input<string>;
    /**
     * Contact number of connection applicant, which is obtained from the account system by default.
     */
    customerContactNumber?: pulumi.Input<string>;
    /**
     * Name of connection applicant, which is obtained from the account system by default.
     */
    customerName?: pulumi.Input<string>;
    /**
     * Connection name.
     */
    directConnectName: pulumi.Input<string>;
    /**
     * Fault reporting contact number.
     */
    faultReportContactNumber?: pulumi.Input<string>;
    /**
     * Fault reporting contact person.
     */
    faultReportContactPerson?: pulumi.Input<string>;
    /**
     * ISP that provides connections. Valid values: ChinaTelecom (China Telecom), ChinaMobile (China Mobile), ChinaUnicom
     * (China Unicom), In-houseWiring (in-house wiring), ChinaOther (other Chinese ISPs), InternationalOperator (international
     * ISPs).
     */
    lineOperator: pulumi.Input<string>;
    /**
     * Local IDC location.
     */
    location?: pulumi.Input<string>;
    /**
     * Port type of connection. Valid values: 100Base-T (100-Megabit electrical Ethernet interface), 1000Base-T (1-Gigabit
     * electrical Ethernet interface), 1000Base-LX (1-Gigabit single-module optical Ethernet interface; 10 KM), 10GBase-T
     * (10-Gigabit electrical Ethernet interface), 10GBase-LR (10-Gigabit single-module optical Ethernet interface; 10 KM).
     * Default value: 1000Base-LX.
     */
    portType: pulumi.Input<string>;
    /**
     * ID of redundant connection.
     */
    redundantDirectConnectId?: pulumi.Input<string>;
    /**
     * Whether the connection applicant has signed the service agreement. Default value: true.
     */
    signLaw?: pulumi.Input<boolean>;
    /**
     * Tencent-side IP address for connection debugging, which is automatically assigned by default.
     */
    tencentAddress?: pulumi.Input<string>;
    /**
     * VLAN for connection debugging, which is enabled and automatically assigned by default.
     */
    vlan?: pulumi.Input<number>;
}
