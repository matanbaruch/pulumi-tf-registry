// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as inputs from "./types/input";
import * as outputs from "./types/output";
import * as utilities from "./utilities";

export class SesSendEmail extends pulumi.CustomResource {
    /**
     * Get an existing SesSendEmail resource's state with the given name, ID, and optional extra
     * properties used to qualify the lookup.
     *
     * @param name The _unique_ name of the resulting resource.
     * @param id The _unique_ provider ID of the resource to lookup.
     * @param state Any extra arguments used during the lookup.
     * @param opts Optional settings to control the behavior of the CustomResource.
     */
    public static get(name: string, id: pulumi.Input<pulumi.ID>, state?: SesSendEmailState, opts?: pulumi.CustomResourceOptions): SesSendEmail {
        return new SesSendEmail(name, <any>state, { ...opts, id: id });
    }

    /** @internal */
    public static readonly __pulumiType = 'tencentcloud:index/sesSendEmail:SesSendEmail';

    /**
     * Returns true if the given object is an instance of SesSendEmail.  This is designed to work even
     * when multiple copies of the Pulumi SDK have been loaded into the same process.
     */
    public static isInstance(obj: any): obj is SesSendEmail {
        if (obj === undefined || obj === null) {
            return false;
        }
        return obj['__pulumiType'] === SesSendEmail.__pulumiType;
    }

    /**
     * Parameters for the attachments to be sent. The TencentCloud API supports a request packet of up to 8 MB in size,and the
     * size of the attachment content will increase by 1.5 times after Base64 encoding. Therefore,you need to keep the total
     * size of all attachments below 4 MB. If the entire request exceeds 8 MB,the API will return an error.
     */
    public readonly attachments!: pulumi.Output<outputs.SesSendEmailAttachment[] | undefined>;
    /**
     * The email address of the cc recipient can support up to 20 cc recipients.
     */
    public readonly bccs!: pulumi.Output<string[] | undefined>;
    /**
     * Cc recipient email address, up to 20 people can be copied.
     */
    public readonly ccs!: pulumi.Output<string[] | undefined>;
    /**
     * Recipient email addresses. You can send an email to up to 50 recipients at a time. Note: the email content will display
     * all recipient addresses. To send one-to-one emails to several recipients, please call the API multiple times to send the
     * emails.
     */
    public readonly destinations!: pulumi.Output<string[]>;
    /**
     * Sender address. Enter a sender address, for example, noreply@mail.qcloud.com.To display the sender name, enter the
     * address in the following format:Sender.
     */
    public readonly fromEmailAddress!: pulumi.Output<string>;
    /**
     * Reply-to address. You can enter a valid personal email address that can receive emails. If this parameter is left empty,
     * reply emails will fail to be sent.
     */
    public readonly replyToAddresses!: pulumi.Output<string | undefined>;
    /**
     * Email subject.
     */
    public readonly subject!: pulumi.Output<string>;
    /**
     * Template parameters for template-based sending. As Simple has been disused, Template is required.
     */
    public readonly template!: pulumi.Output<outputs.SesSendEmailTemplate | undefined>;
    /**
     * Email triggering type. 0 (default): non-trigger-based, suitable for marketing emails and non-immediate emails;1:
     * trigger-based, suitable for immediate emails such as emails containing verification codes.If the size of an email
     * exceeds a specified value,the system will automatically choose the non-trigger-based type.
     */
    public readonly triggerType!: pulumi.Output<number | undefined>;
    /**
     * Unsubscribe link option. 0: Do not add unsubscribe link; 1: English 2: Simplified Chinese; 3: Traditional Chinese; 4:
     * Spanish; 5: French; 6: German; 7: Japanese; 8: Korean; 9: Arabic; 10: Thai.
     */
    public readonly unsubscribe!: pulumi.Output<string | undefined>;

    /**
     * Create a SesSendEmail resource with the given unique name, arguments, and options.
     *
     * @param name The _unique_ name of the resource.
     * @param args The arguments to use to populate this resource's properties.
     * @param opts A bag of options that control this resource's behavior.
     */
    constructor(name: string, args: SesSendEmailArgs, opts?: pulumi.CustomResourceOptions)
    constructor(name: string, argsOrState?: SesSendEmailArgs | SesSendEmailState, opts?: pulumi.CustomResourceOptions) {
        let resourceInputs: pulumi.Inputs = {};
        opts = opts || {};
        if (opts.id) {
            const state = argsOrState as SesSendEmailState | undefined;
            resourceInputs["attachments"] = state ? state.attachments : undefined;
            resourceInputs["bccs"] = state ? state.bccs : undefined;
            resourceInputs["ccs"] = state ? state.ccs : undefined;
            resourceInputs["destinations"] = state ? state.destinations : undefined;
            resourceInputs["fromEmailAddress"] = state ? state.fromEmailAddress : undefined;
            resourceInputs["replyToAddresses"] = state ? state.replyToAddresses : undefined;
            resourceInputs["subject"] = state ? state.subject : undefined;
            resourceInputs["template"] = state ? state.template : undefined;
            resourceInputs["triggerType"] = state ? state.triggerType : undefined;
            resourceInputs["unsubscribe"] = state ? state.unsubscribe : undefined;
        } else {
            const args = argsOrState as SesSendEmailArgs | undefined;
            if ((!args || args.destinations === undefined) && !opts.urn) {
                throw new Error("Missing required property 'destinations'");
            }
            if ((!args || args.fromEmailAddress === undefined) && !opts.urn) {
                throw new Error("Missing required property 'fromEmailAddress'");
            }
            if ((!args || args.subject === undefined) && !opts.urn) {
                throw new Error("Missing required property 'subject'");
            }
            resourceInputs["attachments"] = args ? args.attachments : undefined;
            resourceInputs["bccs"] = args ? args.bccs : undefined;
            resourceInputs["ccs"] = args ? args.ccs : undefined;
            resourceInputs["destinations"] = args ? args.destinations : undefined;
            resourceInputs["fromEmailAddress"] = args ? args.fromEmailAddress : undefined;
            resourceInputs["replyToAddresses"] = args ? args.replyToAddresses : undefined;
            resourceInputs["subject"] = args ? args.subject : undefined;
            resourceInputs["template"] = args ? args.template : undefined;
            resourceInputs["triggerType"] = args ? args.triggerType : undefined;
            resourceInputs["unsubscribe"] = args ? args.unsubscribe : undefined;
        }
        opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts);
        super(SesSendEmail.__pulumiType, name, resourceInputs, opts, false /*dependency*/, utilities.getPackage());
    }
}

/**
 * Input properties used for looking up and filtering SesSendEmail resources.
 */
export interface SesSendEmailState {
    /**
     * Parameters for the attachments to be sent. The TencentCloud API supports a request packet of up to 8 MB in size,and the
     * size of the attachment content will increase by 1.5 times after Base64 encoding. Therefore,you need to keep the total
     * size of all attachments below 4 MB. If the entire request exceeds 8 MB,the API will return an error.
     */
    attachments?: pulumi.Input<pulumi.Input<inputs.SesSendEmailAttachment>[]>;
    /**
     * The email address of the cc recipient can support up to 20 cc recipients.
     */
    bccs?: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * Cc recipient email address, up to 20 people can be copied.
     */
    ccs?: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * Recipient email addresses. You can send an email to up to 50 recipients at a time. Note: the email content will display
     * all recipient addresses. To send one-to-one emails to several recipients, please call the API multiple times to send the
     * emails.
     */
    destinations?: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * Sender address. Enter a sender address, for example, noreply@mail.qcloud.com.To display the sender name, enter the
     * address in the following format:Sender.
     */
    fromEmailAddress?: pulumi.Input<string>;
    /**
     * Reply-to address. You can enter a valid personal email address that can receive emails. If this parameter is left empty,
     * reply emails will fail to be sent.
     */
    replyToAddresses?: pulumi.Input<string>;
    /**
     * Email subject.
     */
    subject?: pulumi.Input<string>;
    /**
     * Template parameters for template-based sending. As Simple has been disused, Template is required.
     */
    template?: pulumi.Input<inputs.SesSendEmailTemplate>;
    /**
     * Email triggering type. 0 (default): non-trigger-based, suitable for marketing emails and non-immediate emails;1:
     * trigger-based, suitable for immediate emails such as emails containing verification codes.If the size of an email
     * exceeds a specified value,the system will automatically choose the non-trigger-based type.
     */
    triggerType?: pulumi.Input<number>;
    /**
     * Unsubscribe link option. 0: Do not add unsubscribe link; 1: English 2: Simplified Chinese; 3: Traditional Chinese; 4:
     * Spanish; 5: French; 6: German; 7: Japanese; 8: Korean; 9: Arabic; 10: Thai.
     */
    unsubscribe?: pulumi.Input<string>;
}

/**
 * The set of arguments for constructing a SesSendEmail resource.
 */
export interface SesSendEmailArgs {
    /**
     * Parameters for the attachments to be sent. The TencentCloud API supports a request packet of up to 8 MB in size,and the
     * size of the attachment content will increase by 1.5 times after Base64 encoding. Therefore,you need to keep the total
     * size of all attachments below 4 MB. If the entire request exceeds 8 MB,the API will return an error.
     */
    attachments?: pulumi.Input<pulumi.Input<inputs.SesSendEmailAttachment>[]>;
    /**
     * The email address of the cc recipient can support up to 20 cc recipients.
     */
    bccs?: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * Cc recipient email address, up to 20 people can be copied.
     */
    ccs?: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * Recipient email addresses. You can send an email to up to 50 recipients at a time. Note: the email content will display
     * all recipient addresses. To send one-to-one emails to several recipients, please call the API multiple times to send the
     * emails.
     */
    destinations: pulumi.Input<pulumi.Input<string>[]>;
    /**
     * Sender address. Enter a sender address, for example, noreply@mail.qcloud.com.To display the sender name, enter the
     * address in the following format:Sender.
     */
    fromEmailAddress: pulumi.Input<string>;
    /**
     * Reply-to address. You can enter a valid personal email address that can receive emails. If this parameter is left empty,
     * reply emails will fail to be sent.
     */
    replyToAddresses?: pulumi.Input<string>;
    /**
     * Email subject.
     */
    subject: pulumi.Input<string>;
    /**
     * Template parameters for template-based sending. As Simple has been disused, Template is required.
     */
    template?: pulumi.Input<inputs.SesSendEmailTemplate>;
    /**
     * Email triggering type. 0 (default): non-trigger-based, suitable for marketing emails and non-immediate emails;1:
     * trigger-based, suitable for immediate emails such as emails containing verification codes.If the size of an email
     * exceeds a specified value,the system will automatically choose the non-trigger-based type.
     */
    triggerType?: pulumi.Input<number>;
    /**
     * Unsubscribe link option. 0: Do not add unsubscribe link; 1: English 2: Simplified Chinese; 3: Traditional Chinese; 4:
     * Spanish; 5: French; 6: German; 7: Japanese; 8: Korean; 9: Arabic; 10: Thai.
     */
    unsubscribe?: pulumi.Input<string>;
}
