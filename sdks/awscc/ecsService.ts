...
export class EcsService extends pulumi.CustomResource {
    ...
    /** @internal */
    public static readonly __pulumiType = 'awscc:index/ecsService:EcsService';
     ...
     * ``defaultCapacityProviderStrategy`` for the cluster is used. A capacity provider strategy may contain a maximum of 6
     * capacity providers.
     */
     ...
     * starting tasks.
     */
    public readonly deploymentConfiguration!: pulumi.Output<outputs.EcsServiceDeploymentConfiguration>;
    /**
     * The deployment controller to use for the service. If no deployment controller is specified, the default value of ``ECS``
     * is used.
     */
    public readonly deploymentController!: pulumi.Output<outputs.EcsServiceDeploymentController>;
    /**
     * The number of instantiations of the specified task definition to place and keep running in your service. For new
     * services, if a desired count is not specified, a default value of ``1`` is used. When using the ``DAEMON`` scheduling
     * strategy, the desired count is not required. For existing services, if a desired count is not specified, it is omitted
     * from the operation.
     */
     ...
     * [Tagging your Amazon ECS resources](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ecs-using-tags.html) in
     * the *Amazon Elastic Container Service Developer Guide*. When you use Amazon ECS managed tags, you need to set the
     * ``propagateTags`` request parameter.
     */
    public readonly enableEcsManagedTags!: pulumi.Output<boolean>;
    /**
     * Determines whether the execute command functionality is turned on for the service. If ``true``, the execute command
     * functionality is turned on for all containers in tasks as part of the service.
     */
    public readonly enableExecuteCommand!: pulumi.Output<boolean>;
    /**
     ...
     * definition health check parameters. For more information, see [Health
     * check](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_HealthCheck.html). If your service's tasks take a
     * while to start and respond to Elastic Load Balancing health checks, you can specify a health check grace period of up to
     * 2,147,483,647 seconds (about 69 years). During that time, the Amazon ECS service scheduler ignores health check status.
     * This grace period can prevent the service scheduler from marking tasks as unhealthy and stopping them before they have
     * time to come up.
     */
    public readonly healthCheckGracePeriodSeconds!: pulumi.Output<number>;
    /**
     ...
     * must be specified as well. For information about the number of load balancers that you can specify per service, see
     * [Service Load Balancing](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/service-load-balancing.html) in the
     * *Amazon Elastic Container Service Developer Guide*.
     */
    public readonly loadBalancers!: pulumi.Output<outputs.EcsServiceLoadBalancer[]>;
    public /*out*/ readonly name!: pulumi.Output<string>;
    /**
     * The network configuration for the service. This parameter is required for task definitions that use the ``awsvpc``
     * network mode to receive their own elastic network interface, and it is not supported for other network modes. For more
     * information, see [Task Networking](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task-networking.html) in
     * the *Amazon Elastic Container Service Developer Guide*.
     */
    public readonly networkConfiguration!: pulumi.Output<outputs.EcsServiceNetworkConfiguration>;
    /**
     ...
     */
    public readonly placementConstraints!: pulumi.Output<outputs.EcsServicePlacementConstraint[]>;
    /**
     * The placement strategy objects to use for tasks in your service. You can specify a maximum of 5 strategy rules for each
     * service.
     */
    public readonly placementStrategies!: pulumi.Output<outputs.EcsServicePlacementStrategy[]>;
    /**
     * The platform version that your tasks in the service are running on. A platform version is specified only for tasks using
     * the Fargate launch type. If one isn't specified, the ``LATEST`` platform version is used. For more information, see
     * [platform versions](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/platform_versions.html) in the *Amazon
     ...
     * the [TagResource](https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_TagResource.html) API action. The
     * default is ``NONE``.
     */
    public readonly propagateTags!: pulumi.Output<string>;
    /**
     ...
     * definition doesn't use the ``awsvpc`` network mode. If you specify the ``role`` parameter, you must also specify a load
     * balancer object with the ``loadBalancers`` parameter. If your account has already created the Amazon ECS service-linked
     * role, that role is used for your service unless you specify a role here. The service-linked role is required if your
     * task definition uses the ``awsvpc`` network mode or if the service is configured to use service discovery, an external
     * deployment controller, multiple target groups, or Elastic Inference accelerators in which case you don't specify a role
     ...
     * Container Service Developer Guide*. If your specified role has a path other than ``/``, then you must either specify the
     * full role ARN (this is recommended) or prefix the role name with the path. For example, if a role with the name ``bar``
     * has a path of ``/foo/`` then you would specify ``/foo/bar`` as the role name. For more information, see [Friendly names
     * and paths](https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_identifiers.html#identifiers-friendly-names) in
     * the *IAM User Guide*.
     */
     ...
     * scheduler strategies available: + ``REPLICA``-The replica scheduling strategy places and maintains the desired number of
     * tasks across your cluster. By default, the service scheduler spreads tasks across Availability Zones. You can use task
     * placement strategies and constraints to customize task placement decisions. This scheduler strategy is required if the
     * service uses the ``CODE_DEPLOY`` or ``EXTERNAL`` deployment controller types. + ``DAEMON``-The daemon scheduling
     * strategy deploys exactly one task on each active container instance that meets all of the task placement constraints
     * that you specify in your cluster. The service scheduler also evaluates the task placement constraints for running tasks
     * and will stop tasks that don't meet the placement constraints. When you're using this strategy, you don't need to
     * specify a desired number of tasks, a task placement strategy, or use Service Auto Scaling policies. Tasks using the
     * Fargate launch type or the ``CODE_DEPLOY`` or ``EXTERNAL`` deployment controller types don't support the ``DAEMON``
     * scheduling strategy.
     */
     ...
     * Tasks can connect to services across all of the clusters in the namespace. Tasks connect through a managed proxy
     * container that collects logs and metrics for increased visibility. Only the tasks that Amazon ECS services create are
     ...
     */
    public readonly serviceConnectConfiguration!: pulumi.Output<outputs.EcsServiceServiceConnectConfiguration>;
    /**
     ...
     * ``ServiceName`` is configured. This is because AWS CloudFormation creates the replacement service first, but each
     * ``ServiceName`` must be unique in the cluster.
     */
    public readonly serviceName!: pulumi.Output<string>;
    /**
     * The details of the service discovery registry to associate with this service. For more information, see [Service
     * discovery](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/service-discovery.html). Each service may be
     * associated with one service registry. Multiple service registries for each service isn't supported.
     */
    public readonly serviceRegistries!: pulumi.Output<outputs.EcsServiceServiceRegistry[]>;
    /**
     ...
     * other services may have restrictions on allowed characters. Generally allowed characters are: letters, numbers, and
     * spaces representable in UTF-8, and the following characters: + - = . _ : / @. + Tag keys and values are case-sensitive.
     * + Do not use ``aws:``, ``AWS:``, or any upper or lowercase combination of such as a prefix for either keys or values as
     * it is reserved for AWS use. You cannot edit or delete tag keys or values with this prefix. Tags with this prefix do not
     * count against your tags per resource limit.
     */
    public readonly tags!: pulumi.Output<outputs.EcsServiceTag[]>;
    /**
     ...
     * uses either the ``ECS`` or ``CODE_DEPLOY`` deployment controllers. For more information about deployment types, see
     ...
     */
    public readonly volumeConfigurations!: pulumi.Output<outputs.EcsServiceVolumeConfiguration[]>;
    ...
    constructor(name: string, argsOrState?: EcsServiceArgs | EcsServiceState, opts?: pulumi.CustomResourceOptions) {
        ...
        if (opts.id) {
            ...
            resourceInputs["volumeConfigurations"] = state ? state.volumeConfigurations : undefined;
        } else {
...
export interface EcsServiceState {
     ...
     * associated with one service registry. Multiple service registries for each service isn't supported.
     */
    serviceRegistries?: pulumi.Input<pulumi.Input<inputs.EcsServiceServiceRegistry>[]>;
    /**
     * The metadata that you apply to the service to help you categorize and organize them. Each tag consists of a key and an
     * optional value, both of which you define. When a service is deleted, the tags are deleted as well. The following basic
     * restrictions apply to tags: + Maximum number of tags per resource - 50 + For each resource, each tag key must be unique,
     ...
     * count against your tags per resource limit.
     */
    tags?: pulumi.Input<pulumi.Input<inputs.EcsServiceTag>[]>;
    /**
     * The ``family`` and ``revision`` (``family:revision``) or full ARN of the task definition to run in your service. If a
     * ``revision`` isn't specified, the latest ``ACTIVE`` revision is used. A task definition must be specified if the service
     * uses either the ``ECS`` or ``CODE_DEPLOY`` deployment controllers. For more information about deployment types, see
     ...
     * Currently, the only supported volume type is an Amazon EBS volume.
     */
    volumeConfigurations?: pulumi.Input<pulumi.Input<inputs.EcsServiceVolumeConfiguration>[]>;
}
