// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as inputs from "./types/input";
import * as outputs from "./types/output";
import * as utilities from "./utilities";

export function getS3AccessPoint(args: GetS3AccessPointArgs, opts?: pulumi.InvokeOptions): Promise<GetS3AccessPointResult> {

    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("awscc:index/getS3AccessPoint:getS3AccessPoint", {
        "id": args.id,
    }, opts, utilities.getPackage());
}

/**
 * A collection of arguments for invoking getS3AccessPoint.
 */
export interface GetS3AccessPointArgs {
    id: string;
}

/**
 * A collection of values returned by getS3AccessPoint.
 */
export interface GetS3AccessPointResult {
    readonly alias: string;
    readonly arn: string;
    readonly bucket: string;
    readonly bucketAccountId: string;
    readonly id: string;
    readonly name: string;
    readonly networkOrigin: string;
    readonly policy: string;
    readonly publicAccessBlockConfiguration: outputs.GetS3AccessPointPublicAccessBlockConfiguration;
    readonly vpcConfiguration: outputs.GetS3AccessPointVpcConfiguration;
}
export function getS3AccessPointOutput(args: GetS3AccessPointOutputArgs, opts?: pulumi.InvokeOptions): pulumi.Output<GetS3AccessPointResult> {
    return pulumi.output(args).apply((a: any) => getS3AccessPoint(a, opts))
}

/**
 * A collection of arguments for invoking getS3AccessPoint.
 */
export interface GetS3AccessPointOutputArgs {
    id: pulumi.Input<string>;
}
