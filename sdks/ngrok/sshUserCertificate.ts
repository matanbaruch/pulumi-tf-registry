...
export class SshUserCertificate extends pulumi.CustomResource {
    ...
    /**
     * the signed SSH certificate in OpenSSH Authorized Keys Format. this value should be placed in a `-cert.pub` certificate
     * file on disk that should be referenced in your `sshd_config` configuration file with a `HostCertificate` directive
     */
     ...
     * `force-command` and `source-address`. See [the OpenSSH certificate protocol
     * spec](https://github.com/openssh/openssh-portable/blob/master/PROTOCOL.certkeys) for additional details.
     */
    ...
    /**
     * A map of extensions included in the certificate. Extensions are additional metadata that can be interpreted by the SSH
     * server for any purpose. These can be used to permit or deny the ability to open a terminal, do port forwarding, x11
     * forwarding, and more. If unspecified, the certificate will include limited permissions with the following extension map:
     * `{"permit-pty": "", "permit-user-rc": ""}` OpenSSH understands a number of predefined extensions. See [the OpenSSH
     * certificate protocol spec](https://github.com/openssh/openssh-portable/blob/master/PROTOCOL.certkeys) for additional
     * details.
     */
    ...
    /**
     * the list of principals included in the ssh user certificate. This is the list of usernames that the certificate holder
     * may sign in as on a machine authorizing the signing certificate authority. Dangerously, if no principals are specified,
     ...
     */
    public readonly validAfter!: pulumi.Output<string>;
    /**
     * the time after which the ssh host certificate becomes invalid, in RFC 3339 format. the OpenSSH certificates RFC calls
     * this `valid_before`.
     */
    ...
    constructor(name: string, argsOrState?: SshUserCertificateArgs | SshUserCertificateState, opts?: pulumi.CustomResourceOptions) {
        ...
        super(SshUserCertificate.__pulumiType, name, resourceInputs, opts, false /*dependency*/, utilities.getPackage());
    }
}
