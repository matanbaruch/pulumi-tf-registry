// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as utilities from "./utilities";

export class DiskStorage extends pulumi.CustomResource {
    /**
     * Get an existing DiskStorage resource's state with the given name, ID, and optional extra
     * properties used to qualify the lookup.
     *
     * @param name The _unique_ name of the resulting resource.
     * @param id The _unique_ provider ID of the resource to lookup.
     * @param state Any extra arguments used during the lookup.
     * @param opts Optional settings to control the behavior of the CustomResource.
     */
    public static get(name: string, id: pulumi.Input<pulumi.ID>, state?: DiskStorageState, opts?: pulumi.CustomResourceOptions): DiskStorage {
        return new DiskStorage(name, <any>state, { ...opts, id: id });
    }

    /** @internal */
    public static readonly __pulumiType = 'commvault:index/diskStorage:DiskStorage';

    /**
     * Returns true if the given object is an instance of DiskStorage.  This is designed to work even
     * when multiple copies of the Pulumi SDK have been loaded into the same process.
     */
    public static isInstance(obj: any): obj is DiskStorage {
        if (obj === undefined || obj === null) {
            return false;
        }
        return obj['__pulumiType'] === DiskStorage.__pulumiType;
    }

    /**
     * Specifies the full path to the storage location.
     */
    public readonly backupLocation!: pulumi.Output<string>;
    /**
     * Specifies the company id to which the created disk storage should be associated with.
     */
    public readonly companyId!: pulumi.Output<number | undefined>;
    /**
     * Specifies the Deduplication path for the storage
     */
    public readonly ddbLocation!: pulumi.Output<string | undefined>;
    /**
     * Specifies the Media agent used for the Disk Storage.
     */
    public readonly mediaagent!: pulumi.Output<string>;
    /**
     * Specifies the Name of the Disk Storage.
     */
    public readonly storageName!: pulumi.Output<string>;

    /**
     * Create a DiskStorage resource with the given unique name, arguments, and options.
     *
     * @param name The _unique_ name of the resource.
     * @param args The arguments to use to populate this resource's properties.
     * @param opts A bag of options that control this resource's behavior.
     */
    constructor(name: string, args: DiskStorageArgs, opts?: pulumi.CustomResourceOptions)
    constructor(name: string, argsOrState?: DiskStorageArgs | DiskStorageState, opts?: pulumi.CustomResourceOptions) {
        let resourceInputs: pulumi.Inputs = {};
        opts = opts || {};
        if (opts.id) {
            const state = argsOrState as DiskStorageState | undefined;
            resourceInputs["backupLocation"] = state ? state.backupLocation : undefined;
            resourceInputs["companyId"] = state ? state.companyId : undefined;
            resourceInputs["ddbLocation"] = state ? state.ddbLocation : undefined;
            resourceInputs["mediaagent"] = state ? state.mediaagent : undefined;
            resourceInputs["storageName"] = state ? state.storageName : undefined;
        } else {
            const args = argsOrState as DiskStorageArgs | undefined;
            if ((!args || args.backupLocation === undefined) && !opts.urn) {
                throw new Error("Missing required property 'backupLocation'");
            }
            if ((!args || args.mediaagent === undefined) && !opts.urn) {
                throw new Error("Missing required property 'mediaagent'");
            }
            if ((!args || args.storageName === undefined) && !opts.urn) {
                throw new Error("Missing required property 'storageName'");
            }
            resourceInputs["backupLocation"] = args ? args.backupLocation : undefined;
            resourceInputs["companyId"] = args ? args.companyId : undefined;
            resourceInputs["ddbLocation"] = args ? args.ddbLocation : undefined;
            resourceInputs["mediaagent"] = args ? args.mediaagent : undefined;
            resourceInputs["storageName"] = args ? args.storageName : undefined;
        }
        opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts);
        super(DiskStorage.__pulumiType, name, resourceInputs, opts, false /*dependency*/, utilities.getPackage());
    }
}

/**
 * Input properties used for looking up and filtering DiskStorage resources.
 */
export interface DiskStorageState {
    /**
     * Specifies the full path to the storage location.
     */
    backupLocation?: pulumi.Input<string>;
    /**
     * Specifies the company id to which the created disk storage should be associated with.
     */
    companyId?: pulumi.Input<number>;
    /**
     * Specifies the Deduplication path for the storage
     */
    ddbLocation?: pulumi.Input<string>;
    /**
     * Specifies the Media agent used for the Disk Storage.
     */
    mediaagent?: pulumi.Input<string>;
    /**
     * Specifies the Name of the Disk Storage.
     */
    storageName?: pulumi.Input<string>;
}

/**
 * The set of arguments for constructing a DiskStorage resource.
 */
export interface DiskStorageArgs {
    /**
     * Specifies the full path to the storage location.
     */
    backupLocation: pulumi.Input<string>;
    /**
     * Specifies the company id to which the created disk storage should be associated with.
     */
    companyId?: pulumi.Input<number>;
    /**
     * Specifies the Deduplication path for the storage
     */
    ddbLocation?: pulumi.Input<string>;
    /**
     * Specifies the Media agent used for the Disk Storage.
     */
    mediaagent: pulumi.Input<string>;
    /**
     * Specifies the Name of the Disk Storage.
     */
    storageName: pulumi.Input<string>;
}
