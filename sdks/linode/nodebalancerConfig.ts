...
export class NodebalancerConfig extends pulumi.CustomResource {
    ...
    /**
     * The type of check to perform against backends to ensure they are serving requests. This is used to determine if backends
     * are up or down. If none no check is performed. connection requires only a connection to the backend to succeed. http and
     * http_body rely on the backend serving HTTP, and that the response returned matches what is expected.
     */
    ...
    /**
     * What ciphers to use for SSL connections served by this NodeBalancer. `legacy` is considered insecure and should only be
     * used if necessary.
     */
    ...
    /**
     * The TCP port this Config is for. These values must be unique across configs on a single NodeBalancer (you can't have two
     * configs for port 80, for example). While some ports imply some protocols, no enforcement is done and you may configure
     * your NodeBalancer however is useful to you. For example, while port 443 is generally used for HTTPS, you do not need SSL
     * configured to have a NodeBalancer listening on port 443.
     */
    ...
    /**
     * The version of ProxyProtocol to use for the underlying NodeBalancer. This requires protocol to be `tcp`. Valid values
     * are `none`, `v1`, and `v2`.
     */
    public readonly proxyProtocol!: pulumi.Output<string>;
    /**
     * The certificate this port is serving. This is not returned. If set, this field will come back as `<REDACTED>`. Please
     * use the ssl_commonname and ssl_fingerprint to identify the certificate.
     */
    ...
    constructor(name: string, argsOrState?: NodebalancerConfigArgs | NodebalancerConfigState, opts?: pulumi.CustomResourceOptions) {
        ...
        super(NodebalancerConfig.__pulumiType, name, resourceInputs, opts, false /*dependency*/, utilities.getPackage());
    }
}
