...
export class Client extends pulumi.CustomResource {
    ...
    /**
     * Type of application the client represents. Possible values are: `native`, `spa`, `regular_web`, `non_interactive`,
     * `sso_integration`. Specific SSO integrations types accepted as well are: `rms`, `box`, `cloudbees`, `concur`, `dropbox`,
     * `mscrm`, `echosign`, `egnyte`, `newrelic`, `office365`, `salesforce`, `sentry`, `sharepoint`, `slack`, `springcm`,
     * `zendesk`, `zoom`.
     */
    public readonly appType!: pulumi.Output<string | undefined>;
    /**
     * URLs that Auth0 may call back to after a user authenticates for the client. Make sure to specify the protocol (https://)
     * otherwise the callback may fail in some cases. With the exception of custom URI schemes for native clients, all
     * callbacks should use protocol https://.
     */
    ...
    /**
     * Metadata associated with the client, in the form of an object with string values (max 255 chars). Maximum of 10 metadata
     * properties allowed. Field names (max 255 chars) are alphanumeric and may only include the following special characters:
     * `:,-+=_*?"/\()<>@ [Tab] [Space]`.
     */
    public readonly clientMetadata!: pulumi.Output<{[key: string]: string} | undefined>;
    /**
     * Whether this client can be used to make cross-origin authentication requests (`true`) or it is not allowed to make such
     * requests (`false`).
     */
    public readonly crossOriginAuth!: pulumi.Output<boolean | undefined>;
    /**
     * URL of the location in your site where the cross-origin verification takes place for the cross-origin auth flow when
     * performing authentication in your own domain instead of Auth0 Universal Login page.
     */
    ...
    /**
     * Indicates whether the token endpoint IP header is trusted. Requires the authentication method to be set to
     * `client_secret_post` or `client_secret_basic`. Setting this property when creating the resource, will default the
     * authentication method to `client_secret_post`. To change the authentication method to `client_secret_basic` use the
     * `auth0.ClientCredentials` resource.
     */
     ...
     */
    public readonly jwtConfiguration!: pulumi.Output<outputs.ClientJwtConfiguration | undefined>;
    /**
     * URL of the logo for the client. Recommended size is 150px x 150px. If none is set, the default badge for the application
     * type will be shown.
     */
    ...
    /**
     * Configuration settings to toggle native social login for mobile native applications. Once this is set it must stay set,
     * with both resources set to `false` in order to change the `app_type`.
     */
    public readonly nativeSocialLogin!: pulumi.Output<outputs.ClientNativeSocialLogin | undefined>;
    /**
    ...
    /**
     * Defines how to proceed during an authentication transaction when `organization_usage = "require"`. Can be `no_prompt`
     * (default), `pre_login_prompt` or `post_login_prompt`.
     */
    public readonly organizationRequireBehavior!: pulumi.Output<string | undefined>;
    /**
     * Defines how to proceed during an authentication transaction with regards to an organization. Can be `deny` (default),
     * `allow` or `require`.
     */
     ...
     */
    public readonly refreshToken!: pulumi.Output<outputs.ClientRefreshToken | undefined>;
    /**
     * Makes the use of Pushed Authorization Requests mandatory for this client. This feature currently needs to be enabled on
     * the tenant in order to make use of it.
     */
    ...
    /**
     * Applies only to SSO clients and determines whether Auth0 will handle Single Sign-On (true) or whether the identity
     * provider will (false).
     */
