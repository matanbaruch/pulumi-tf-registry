...
export class ApplicationSettings extends pulumi.CustomResource {
    ...
    /**
     * Set the duration for which the jobs are considered as old and expired. After that time passes, the jobs are archived and
     * no longer able to be retried. Make it empty to never expire jobs. It has to be no less than 1 day, for example: 15 days,
     * 1 month, 2 years.
     */
    ...
    /**
     * By default, we write to the authorized_keys file to support Git over SSH without additional configuration. GitLab can be
     * optimized to authenticate SSH keys via the database file. Only disable this if you have configured your OpenSSH server
    ...
    /**
     * Enable Auto DevOps for projects by default. It automatically builds, tests, and deploys applications based on a
    ...
    /**
     * Enabling this makes only licensed EE features available to projects if the project namespace’s plan includes the
    ...
    /**
     * Determine if developers can push to the default branch. Can take: 0 (not protected, both users with the Developer role
     * or Maintainer role can push new commits and force push), 1 (partially protected, users with the Developer role or
     * Maintainer role can push new commits, but cannot force push) or 2 (fully protected, users with the Developer or
     * Maintainer role cannot push new commits, but users with the Developer or Maintainer role can; no one can force push) as
     * a parameter. Default is 2.
     */
    ...
    /**
     * Enable inactive project deletion feature. Introduced in GitLab 14.10. Became operational in GitLab 15.0 (with feature
     * flag inactive_projects_deletion).
     */
    public readonly deleteInactiveProjects!: pulumi.Output<boolean>;
    /**
     * The number of days to wait before deleting a project or group that is marked for deletion. Value must be between 1 and
     * 90.
     */
    ...
    /**
     * Users with email addresses that match these domains cannot sign up. Wildcards allowed. Use separate lines for multiple
    ...
    /**
     * The minimum allowed curve size (in bits) of an uploaded ED25519_SK key. 0 means no restriction. -1 disables ED25519_SK
     * keys.
     */
    ...
    /**
     * Maximum size of text fields to index by Elasticsearch. 0 value means no limit. This does not apply to repository and
     * wiki indexing.
     */
    ...
    /**
     * Maximum concurrency of Elasticsearch bulk requests per indexing operation. This only applies to repository indexing
     * operations.
     */
    ...
    /**
     * Some email servers do not support overriding the email sender name. Enable this option to include the name of the author
     * of the issue, merge request or comment in the email body instead.
     */
    ...
    /**
     * (If enabled, requires: external_auth_client_key) The certificate to use to authenticate with the external authorization
     * service.
     */
    public readonly externalAuthClientCert!: pulumi.Output<string>;
    /**
     * Private key for the certificate when authentication is required for the external authorization service, this is
    ...
    /**
     * The default classification label to use when requesting authorization and no classification label has been specified on
     * the project.
     */
    public readonly externalAuthorizationServiceDefaultLabel!: pulumi.Output<string>;
    /**
     * (If enabled, requires: external_authorization_service_default_label, external_authorization_service_timeout and
     * external_authorization_service_url) Enable using an external authorization service for accessing projects.
     */
    public readonly externalAuthorizationServiceEnabled!: pulumi.Output<boolean>;
    /**
     * The timeout after which an authorization request is aborted, in seconds. When a request times out, access is denied to
     * the user. (min: 0.001, max: 10, step: 0.001).
     */
    ...
    /**
     * Start day of the week for calendar views and date pickers. Valid values are 0 for Sunday, 1 for Monday, and 6 for
     * Saturday.
     */
    ...
    /**
     * Medium Gitaly timeout, in seconds. This should be a value between the Fast and the Default timeout. Set to 0 to disable
     * timeouts.
     */
     ...
     * repositories on disk. This prevents repositories from having to be moved or renamed when the Project URL changes and may
     * improve disk I/O performance. (Always enabled in GitLab versions 13.0 and later, configuration is scheduled for removal
     * in 14.0).
     */
    ...
    /**
     * Enable or disable Git housekeeping. If enabled, requires either housekeeping_optimize_repository_period OR
     * housekeeping_bitmaps_enabled, housekeeping_full_repack_period, housekeeping_gc_period, and
     * housekeeping_incremental_repack_period. Options housekeeping_bitmaps_enabled, housekeeping_full_repack_period,
     * housekeeping_gc_period, and housekeeping_incremental_repack_period are deprecated. Use
     * housekeeping_optimize_repository_period instead.
     */
    ...
    /**
     * Sources to allow project import from. Valid values are: `github`, `bitbucket`, `bitbucket_server`, `fogbugz`, `git`,
     * `gitlab.Project`, `gitea`, `manifest`
     */
    ...
    /**
     * If delete_inactive_projects is true, the time (in months) to wait before deleting inactive projects. Introduced in
     * GitLab 14.10. Became operational in GitLab 15.0.
     */
    public readonly inactiveProjectsDeleteAfterMonths!: pulumi.Output<number>;
    /**
     * If delete_inactive_projects is true, the minimum repository size for projects to be checked for inactivity. Introduced
     * in GitLab 14.10. Became operational in GitLab 15.0.
     */
    public readonly inactiveProjectsMinSizeMb!: pulumi.Output<number>;
    /**
     * If delete_inactive_projects is true, sets the time (in months) to wait before emailing maintainers that the project is
     * scheduled be deleted because it is inactive. Introduced in GitLab 14.10. Became operational in GitLab 15.0.
     */
    ...
    /**
     * When instance is in maintenance mode, non-administrative users can sign in with read-only access and make read-only API
     * requests.
     */
    ...
    /**
     * Maximum number of unique repositories a user can download in the specified time period before they are banned. Maximum:
     * 10,000 repositories. Introduced in GitLab 15.1.
     */
    ...
    /**
     * Allow repository mirroring to configured by project Maintainers. If disabled, only Administrators can configure
    ...
    /**
     * Define a list of trusted domains or IP addresses to which local requests are allowed when local requests for hooks and
    ...
    /**
     * Require users to prove ownership of custom domains. Domain verification is an essential security measure for public
     * GitLab sites. Users are required to demonstrate they control a domain before it is enabled.
     */
    ...
    /**
     * Number of changes (branches or tags) in a single push to determine whether individual push events or bulk push events
     * are created. Bulk push events are created if it surpasses that value.
     */
    public readonly pushEventActivitiesLimit!: pulumi.Output<number>;
    /**
     * Number of changes (branches or tags) in a single push to determine whether webhooks and services fire or not. Webhooks
     * and services aren’t submitted if it surpasses that value.
     */
    ...
    /**
     * (GitLab 13.0 and earlier) List of names of enabled storage paths, taken from gitlab.yml. New projects are created in one
     * of these stores, chosen at random.
     */
    public readonly repositoryStorages!: pulumi.Output<string[]>;
    /**
     * (GitLab 13.1 and later) Hash of names of taken from gitlab.yml to weights. New projects are created in one of these
     * stores, chosen by a weighted random selection.
     */
    public readonly repositoryStoragesWeighted!: pulumi.Output<{[key: string]: number}>;
    /**
     * When enabled, any user that signs up for an account using the registration form is placed under a Pending approval state
     * and has to be explicitly approved by an administrator.
     */
    ...
    /**
     * Selected levels cannot be used by non-Administrator users for groups, projects or snippets. Can take private, internal
     * and public as a parameter. Null means there is no restriction.
     */
    ...
    /**
     * URL of the external Spamcheck service endpoint. Valid URI schemes are grpc or tls. Specifying tls forces communication
     * to be encrypted.
     */
    ...
    /**
     * (If enabled, requires: throttle_authenticated_api_period_in_seconds and throttle_authenticated_api_requests_per_period)
     ...
     * throttle_authenticated_packages_api_requests_per_period) Enable authenticated API request rate limit. Helps reduce
     * request volume (for example, from crawlers or abusive bots). View Package Registry rate limits for more details.
     */
    ...
    /**
     * (If enabled, requires: throttle_authenticated_web_period_in_seconds and throttle_authenticated_web_requests_per_period)
     ...
     * throttle_unauthenticated_api_requests_per_period) Enable unauthenticated API request rate limit. Helps reduce request
     ...
     * throttle_unauthenticated_packages_api_requests_per_period) Enable authenticated API request rate limit. Helps reduce
     * request volume (for example, from crawlers or abusive bots). View Package Registry rate limits for more details.
     */
     ...
     * throttle_unauthenticated_web_requests_per_period) Enable unauthenticated web request rate limit. Helps reduce request
     * volume (for example, from crawlers or abusive bots).
     */
    ...
    /**
     * When set to false disable the You won't be able to pull or push project code via SSH warning shown to users with no
     * uploaded SSH key.
     */
