...
export class Project extends pulumi.CustomResource {
     ...
     * with the `gitlab.ProjectApprovalRule` resource and is most likely gonna be deprecated in a future GitLab version (see
     * [this upstream epic](https://gitlab.com/groups/gitlab-org/-/epics/7572)). In the meantime we recommend against using
     * this attribute and use `gitlab.ProjectApprovalRule` instead.
     */
    ...
    /**
     * The role required to cancel a pipeline or job. Introduced in GitLab 16.8. Premium and Ultimate only. Valid values are
     ...
     */
    public readonly containerExpirationPolicy!: pulumi.Output<outputs.ProjectContainerExpirationPolicy | undefined>;
    /**
    ...
    /**
     * Create main branch with first commit containing a README.md file. Must be set to `true` if importing an uninitialized
     * project with a different `default_branch`.
     */
     ...
     */
    public readonly pushRules!: pulumi.Output<outputs.ProjectPushRules | undefined>;
    /**
     ...
     * since template_name may be ambiguous (enterprise edition). This option is mutually exclusive with `template_name`. See
     * `gitlab.GroupProjectFileTemplate` to set a project as a template project. If a project has not been set as a template,
    ...
    /**
     * Use either custom instance or group (with group_with_project_templates_id) project template (enterprise edition). > When
     * using a custom template, [Group Tokens won't
     * work](https://docs.gitlab.com/15.7/ee/user/project/settings/import_export_troubleshooting.html#import-using-the-rest-api-fails-when-using-a-group-access-token).
     * You must use a real user's Personal Access Token.
     */
