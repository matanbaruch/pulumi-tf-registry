...
export class LdapGroupSettingV2 extends pulumi.CustomResource {
    ...
    /**
     * This attribute is used in very specific cases of LDAP group settings. Don't switch it to `false`, unless instructed by
     * the JFrog support team. Default value is `false`.
     */
     ...
     * the user is a member. HIERARCHICAL: The user's DN is indicative of the groups the user belongs to by using group names
     * as part of user DN hierarchy. Each user DN contains a list of ou's or custom attributes that make up the group
     * association. For example, `uid=user1,ou=developers,ou=uk,dc=jfrog,dc=org` indicates that `user1` belongs to two groups:
     * `uk` and `developers`. Valid values are: `STATIC`, `DYNAMIC`, `HIERARCHICAL`, case sensitive, all caps.
     */
    public readonly strategy!: pulumi.Output<string>;
    /**
     * When set, enables deep search through the sub-tree of the LDAP URL + Search Base. `true` by default. `sub_tree` can be
     * set to true only with `STATIC` or `DYNAMIC` strategy.
     */
    ...
    constructor(name: string, argsOrState?: LdapGroupSettingV2Args | LdapGroupSettingV2State, opts?: pulumi.CustomResourceOptions) {
        ...
        super(LdapGroupSettingV2.__pulumiType, name, resourceInputs, opts, false /*dependency*/, utilities.getPackage());
    }
}
